...
var handleInput21 = function (input, k,adjustW) {
    let planes = [];
...
var getPara = function (input, newSize, size) {
    ...
    let ch = (newSize.height-size.height) / size.height;
    let cv = (newSize.width-size.width) / size.width;
    let ch2 = newSize.height / size.height;
    let cv2 = newSize.width / size.width;
    if (ch == cv) {
        amplify = newSize.height/size.height;
        orit = 'equal'
        distortion = 1;
